...
public class CoverageSensorTest {
  ...
  public void init() {
    ...
    settings.setProperty(TypeScriptPlugin.SETTING_LCOV_UT_REPORT_PATH, "test_lcov_report_ut.dat");
    settings.setProperty(TypeScriptPlugin.SETTING_LCOV_IT_REPORT_PATH, "test_lcov_report_it.dat");
  ...
  public void test_should_execute() {
    ...
    fs.add(new DefaultInputFile("fake_file.ts").setType(InputFile.Type.MAIN).setLanguage(TypeScriptLanguage.LANGUAGE_KEY));
    assertThat(localSensor.shouldExecuteOnProject(project)).isTrue();

    // no path to report -> do execute
    localSettings.setProperty(TypeScriptPlugin.SETTING_LCOV_UT_REPORT_PATH, "");
    assertThat(localSensor.shouldExecuteOnProject(project)).isTrue();
  }
  ...
  public void report_not_found() throws Exception {
    ...
    Settings localSettings = new Settings();
    localSettings.setProperty(TypeScriptPlugin.SETTING_LCOV_UT_REPORT_PATH, "/fake/path/lcov_report.dat");
  ...
  public void test_file_in_ut_coverage_report() {
    DefaultFileSystem fs = newFileSystem();
    fs.add(newSourceInputFile("fake_file1.ts"));
    fs.add(newSourceInputFile("fake_file2.ts"));
    fs.add(newSourceInputFile("fake_file3.ts"));
  ...
  public void test_wrong_lines_in_file() {
    settings.setProperty(TypeScriptPlugin.SETTING_LCOV_UT_REPORT_PATH, "wrong_line_lcov.info");
    DefaultFileSystem fs = newFileSystem();
    fs.add(newSourceInputFile("fake_file.ts"));
    newUTSensor(fs, settings).analyse(project, context);
  ...
  public void test_file_not_in_coverage_report() {
    DefaultFileSystem fs = newFileSystem();
    fs.add(newSourceInputFile("not_in_report.ts"));
  ...
  public void save_zero_value_for_all_files_when_no_report() throws Exception {
    ...
    settings.setProperty(TypeScriptPlugin.SETTING_FORCE_ZERO_COVERAGE, "true");
    settings.setProperty(TypeScriptPlugin.SETTING_LCOV_UT_REPORT_PATH, "");
  ...
  public DefaultFileSystem newFileSystem() {
    DefaultFileSystem fs = new DefaultFileSystem();
    fs.setBaseDir(TestUtils.getResource("com/pablissimo/sonar/lcov"));
